...
class OBSWebsocketsManager:
    ...
    def __init__(self):
        ...
        self.obsws = obsws(WEBSOCKET_HOST, WEBSOCKET_PORT, WEBSOCKET_PASSWORD)
        try:
            self.obsws.connect()
        except:
    ...
    def set_scene(self, new_scene):
        self.obsws.call(requests.SetCurrentProgramScene(sceneName=new_scene))

    # Set the visibility of any source's filters
    def set_filter_visibility(self, source_name, filter_name, filter_enabled=True):
        self.obsws.call(requests.SetSourceFilterEnabled(sourceName=source_name, filterName=filter_name, filterEnabled=filter_enabled))

    # Set the visibility of any source
    def set_source_visibility(self, scene_name, source_name, source_visible=True):
        response = self.obsws.call(requests.GetSceneItemId(sceneName=scene_name, sourceName=source_name))
        myItemID = response.datain['sceneItemId']
        self.obsws.call(requests.SetSceneItemEnabled(sceneName=scene_name, sceneItemId=myItemID, sceneItemEnabled=source_visible))

    # Returns the current text of a text source
    def get_text(self, source_name):
        response = self.obsws.call(requests.GetInputSettings(inputName=source_name))
        return response.datain["inputSettings"]["text"]

    # Returns the text of a text source
    def set_text(self, source_name, new_text):
        self.obsws.call(requests.SetInputSettings(inputName=source_name, inputSettings = {'text': new_text}))

    def get_source_transform(self, scene_name, source_name):
        response = self.obsws.call(requests.GetSceneItemId(sceneName=scene_name, sourceName=source_name))
        myItemID = response.datain['sceneItemId']
        response = self.obsws.call(requests.GetSceneItemTransform(sceneName=scene_name, sceneItemId=myItemID))
        transform = {}
    ...
    def set_source_transform(self, scene_name, source_name, new_transform):
        response = self.obsws.call(requests.GetSceneItemId(sceneName=scene_name, sourceName=source_name))
        myItemID = response.datain['sceneItemId']
        self.obsws.call(requests.SetSceneItemTransform(sceneName=scene_name, sceneItemId=myItemID, sceneItemTransform=new_transform))
    ...
    def get_input_settings(self, input_name):
        return self.obsws.call(requests.GetInputSettings(inputName=input_name))

    # Get list of all the input types
    def get_input_kind_list(self):
        return self.obsws.call(requests.GetInputKindList())

    # Get list of all items in a certain scene
    def get_scene_items(self, scene_name):
        return self.obsws.call(requests.GetSceneItemList(sceneName=scene_name))
...
if __name__ == '__main__':
    ...
    #cur_scene = 'Face Stream'
    cur_scene = 'Vtuber Stream'
    obswebsockets_manager.set_scene(cur_scene)
    obswebsockets_manager.set_source_visibility(cur_scene, "Head Text", False)
    obswebsockets_manager.set_text("Head Text", "Hai :3")
    time.sleep(2)
    obswebsockets_manager.set_source_visibility(cur_scene, "Head Text", True)
    time.sleep(2)
    obswebsockets_manager.set_text("Head Text", "Alec go fuck\nyourself")
    time.sleep(2)
    obswebsockets_manager.set_source_visibility(cur_scene, "Head Text", False)
